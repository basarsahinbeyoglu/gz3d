...
GZ3D.Scene = function (shaders, camera_type, fov, camera_target_offset, camera_yaw, camera_pitch, camera_distance, follow_model, interpolate_camera_rate,
                       interp_model, interpolate_position_rate, interpolate_rotation_rate, interpolate_snap_distance, fog_distance, fog_color) {
  this.emitter = globalEmitter || new EventEmitter2({ verboseMemoryLeak: true });
  this.shaders = shaders;

  // read all the parameters and set defaults if not set
  this.cameraType = camera_type; // fixed (yaw/pitch constant) or follow (must set followModel and yaw/pitch relative to target object)
  ...
  this.cameraYaw = camera_yaw ? camera_yaw : 0.0; // default yaw 0
  this.cameraPitch = camera_pitch ? camera_pitch : -Math.PI * 0.25; // default pitch is looking down 45 degrees
  this.cameraDistance = camera_distance ? camera_distance : 10.0; // how far camera is from target
  this.followModel = follow_model; // if set, the camera stays locked on this object

  this.interpModel = interp_model; // if set, this object will interpolate to smooth out lags
  // If following an object set interpModel. We probably always want this.
  // However if we have a use case in the future where we want to follow an object without interpolation we can remove this.
  // Then we will set followModel and interpModel separately as needed
  if (this.followModel) {
    this.interpModel = this.followModel;
  }

  // by default interpolation is on for follow camera (as we had for drone challenge) but off for other cameras - lots of issues still to consider
  var defaultInterpolationRate = (cameraType == "follow") ? 0.1 : 1.0;
  this.interpolatePositionRate = interpolate_position_rate ? interpolate_position_rate : defaultInterpolationRate; // how fast to interpolate the position of target object (if interpolating)
  this.interpolateRotationRate = interpolate_rotation_rate ? interpolate_rotation_rate : defaultInterpolationRate; // how fast to interpolate the rotation of target object (if interpolating)
  this.interpolateCameraRate = interpolate_camera_rate ? interpolate_camera_rate : defaultInterpolationRate; // how fast camera position and camera lookAt point interpolate

  // This is an annoying thing to have and hopefully can be improved later
  // When user resets we need to know (A) reset hapens (B) when the object actually gets new position
  // Then we could snap the interpolations so we don't see robot and camera slide across the world.
  // However this gets complicated because we may never see the original starting point due to lag.
  // The current solution is just check if robot moved more than a set distance and snap interpolation (assume it was a reset).

  this.interpolateSnapDistance = interpolate_snap_distance ? interpolate_snap_distance : 10.0; // this will hopefully be temporary
  this.fogDistance = fog_distance;
  this.fogColor = fog_color;

  // Temporary Code: For Drone Challenge backwards compatibility
  // After we merge and update Drone projects this block of code can be deleted.
  if (this.followModel && this.cameraType == null) {
    this.cameraType = "follow";
    this.cameraPitch = -0.38;
    this.cameraDistance = 0.269;
    this.interpModel = this.followModel;
    this.interpolatePositionRate = 0.1;
    this.interpolateRotationRate = 0.1;
    this.interpolateCameraRate = 0.1;
  }
...
GZ3D.Scene.prototype.init = function () {
  ...
  if (this.fogDistance) { // add fog if parameter set
    fog_three_color = null;
    if (this.fogColor == null) { // if no fog color set use default white
      fog_three_color = new THREE.Color(0xffffff);
    }
    else { // use the parameter fog color
      fog_three_color = new THREE.Color(this.fogColor[0], this.fogColor[1], this.fogColor[2]);
    }

    this.scene.fog = new THREE.Fog(fog_three_color, this.fogDistance[0], this.fogDistance[1]);
    this.scene.background = fog_three_color; // must set background to match fog color for smooth blend
    this.camera.far = this.fogDistance[1]; // set far distance to match end of the fog
  }
};
...
GZ3D.Scene.prototype.render = function () {
  ...
  // if we temporarily modify the position of the target object (to smooth update lags)
  // we will restore it after the render loop
  var save_model_position;
  var save_model_quaternion;
  if (this.interpModelObject) {
    save_model_quaternion = this.interpModelObject.quaternion.clone();
    save_model_position = this.interpModelObject.position.clone();
  }

  // Reset interpolation when interp object is beyond an expected distance. This needs refinement.
  // We need to know when user brought window back into view and also if reset happens, we need to know
  // when the actual position change of the robot reaches the client and reset then.
  if (this.interpModelObject) {
    if (this.last_interpModelObject_position != null &&
      this.last_interpModelObject_position.distanceTo(this.interpModelObject.position) > this.interpolateSnapDistance) {
      // camera interpolations needs to reset
      this.interp_object_position = null;
      this.interp_object_quaternion = null;
      this.interp_camera_offset = null;
      this.interp_camera_lookAt = null;
    }
    this.last_interpModelObject_position = this.interpModelObject.position;
  }

  // if we did not set camera in the parameters then we do the default camera
  // if we have a fixed camera are we are not following an object, only update the first frame
  // the camera will never move again - this allows the user to still have mouse control if they want.
  // If we are following an object, we are effectively disabling mouse control.
  if (this.cameraType && // if no camera set, skip all of it - we use the default camera
      (this.interp_camera_offset == null || this.followModelObject != null) && // only update first frame if we are not following an object - let's user keep mouse control
      (this.followModelObject != null || this.followModel == null)) { // if there is a follow object, don't update until we see it - otherwise we'll see camera move from origin to start position
    // there are 3 possibilities
    //    (1)  camera=fixed  followModel=Not Set      - means we target the world origin (perhaps with a specified offset)
    //    (2)  camera=fixed  followModel=something    - means we target the object (perahsp with a specified offset)
    //    (3)  camera=follow followModel=something    - means we follow this object in 3rd person (drone challenge)

    // now calculate the position the camera should look at
    var camera_look_at_position = new THREE.Vector3(0.0, 0.0, 0.0) // default is to be origin
    if (this.followModelObject) {
      camera_look_at_position = this.followModelObject.position.clone();
    }

    // In all case we will apply cameraYaw, cameraPitch, and cameraDistance
    // For (3) cameraYaw and cameraPitch are relative to the object, for (1) and (2) they are relative to the world

    // Determine the basis vector and rotate it by yaw and pitch
    // define the basis for rotation and apply rotation for yaw and pitch
    if (this.followModelObject && this.cameraType == "follow") {
      var xAxis = new THREE.Vector3(0, 0, 0);
      var yAxis = new THREE.Vector3(0, 0, 0);
      var zAxis = new THREE.Vector3(0, 0, 0);
      this.followModelObject.matrix.extractBasis(xAxis, yAxis, zAxis);
      var camera_set_position = xAxis.multiplyScalar(-this.cameraDistance);
      // rotate camera_set_position around the basic vectors for yaw and pitch
      camera_set_position.applyAxisAngle(yAxis, -this.cameraPitch);
      camera_set_position.applyAxisAngle(zAxis, this.cameraYaw); // for follow camera there is no reason this would not be 0
    }
    else {
      var camera_set_position = new THREE.Vector3(0.0, -this.cameraDistance, 0.0);
      // for fixed camera we use the axis basis which is matched to the default camera setup
      var xAxis = new THREE.Vector3(1, 0, 0);
      var yAxis = new THREE.Vector3(0, 1, 0);
      camera_set_position.applyAxisAngle(xAxis, this.cameraPitch);
      camera_set_position.applyAxisAngle(yAxis, this.cameraYaw);
    }

    // then define it relative to the look at position
    camera_set_position.add(camera_look_at_position);

    // lerp camera position
    if (this.interp_camera_offset == null) {
      this.interp_camera_offset = camera_set_position.clone();
    }
    else {
      this.interp_camera_offset.lerp(camera_set_position, this.interpolateCameraRate);
    }

    // lerp camera look at position
    if (this.interp_camera_lookAt == null) {
      this.interp_camera_lookAt = camera_look_at_position.clone()
    }
    else {
      this.interp_camera_lookAt.lerp(camera_look_at_position, this.interpolateCameraRate);
    }

    // set the final position of the camera
    // We always apply the cameraTargetOffset
    // For (1) this is relative to the world origin
    // For (2) and (3) this is relative to the follow object position
    this.camera.position.copy(this.interp_camera_offset.clone().add(this.cameraTargetOffset));
    this.camera.lookAt(this.interp_camera_lookAt.clone().add(this.cameraTargetOffset));
    this.camera.updateMatrix();
  }

  // now handle interpolation of the target object
  if(this.interpModelObject) {
    // First we interpolate the rotation
    if (this.interp_object_quaternion == null) {
      this.interp_object_quaternion = this.interpModelObject.quaternion.clone();
    }
    else {
      var result = new THREE.Quaternion();
      THREE.Quaternion.slerp(this.interp_object_quaternion, this.interpModelObject.quaternion, result, this.interpolateRotationRate);
      this.interp_object_quaternion.copy(result);
      this.interpModelObject.quaternion.copy(this.interp_object_quaternion);
      this.interpModelObject.updateMatrix();
    }

    // Now handle the interp of the position.
    // If we are following this same object we will simply lock the object to the camera look at position.
    // Otherwise we do a simple lerp
    if (this.interp_camera_lookAt != null && this.followModelObject != null && this.cameraType == "follow") {
      // we are following the object with the camera so we just lock to the camera look at position
      // the avoids a very jarring pop between the two though we may relax this later and allow the object
      // to 'float' a bit relative to the camera - I think this can be nice for things like the drone but did not try yet
      this.followModelObject.position.copy(this.interp_camera_lookAt);
    }
    else
    {
      // there is no follow camera but we still want to smooth out the object
      if (this.interp_object_position == null) {
        this.interp_object_position = this.interpModelObject.position.clone();
      }
      else {
        this.interp_object_position.lerp(this.interpModelObject.position, this.interpolatePositionRate);
        this.interpModelObject.position.copy(this.interp_object_position);
      }
    }
  }
  ...
  // restore the interpolated object position
  if (this.interpModelObject) {
    this.interpModelObject.position.copy(save_model_position);
    this.interpModelObject.quaternion.copy(save_model_quaternion);
    this.interpModelObject.updateMatrix();
  }
};
...
GZ3D.Scene.prototype.add = function (model) {
  ...
  // save ptr to object when we come across it
  if (this.followModel && model.name == this.followModel) {
    this.followModelObject = model;
  }
  if (this.interpModel && model.name == this.interpModel) {
    this.interpModelObject = model;
  }
};
...
GZ3D.Scene.prototype.updatePose = function (model, position, orientation) {
  if (this.modelManipulator && this.modelManipulator.object && this.modelManipulator.hovered) {
    return;
  }
    ...
    for (var j = 0; j < model.children.length; ++j) {
      ...
      if (inertial) {
        ...
        if (mass < 0 || Ixx < 0 || Iyy < 0 || Izz < 0 ||
          ...
          material = {
            'ambient': [1, 0.0, 1, 1], 'diffuse': [1, 0.0, 1, 1],
